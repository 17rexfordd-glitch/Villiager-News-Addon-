...
public final class DialogueSubtitleState {
	...
	public static void tick(Minecraft minecraft) {
		...
		while (iterator.hasNext()) {
			...
			Entity entity = ClientEntityLookup.byUuid(minecraft.level, entry.getKey());
	...
	private static Component subtitleLine(Entity entity, Component transcript) {
		...
		if (entity instanceof Villager villager && !villager.hasCustomName()) {
			name = Component.literal(BuiltInRegistries.VILLAGER_PROFESSION.getKey(villager.getVillagerData().getProfession()).getPath());
		}
